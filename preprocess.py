...
class Connection:
    ...
    def clean_clinic_data(self, miss_rate=0.2, cleaned_version=False, max_gcs=15,
                          ext_subjects=True):
        ...
        dropping_cols = ['EDDrugScreenAlcoholDone', 'GcsEDArrScore', 'HospDischTimeSinceInj', 'GcsCompScoresArrivUnavail', 'Cohort',
                         ...
                         'EDArrPupilReactivityLghtLftEyeReslt', 'EDArrPupilReactivityLghtRtEyeReslt']
        ...
        # Dropping null rows (for now, only GOSEScore)
        self.clinic_data.dropna(axis=0, subset=['GOSEScore'], inplace=True)
    ...
    def clean_vital_data(self):
        ...
        self.vital_data.loc[
            self.vital_data['DVRespSuppTyp'].isnull(), self.vital_data.columns.str.startswith('DVResTyp' + "_")] = np.nan
    ...
    def clean_gcs_data(self):
        ...
        def aggregate_pupils(row):
            ...
            if row['DVPupilReactivityLghtLftEyeReslt'] == 'Untestable' or row[
                ...
                res = 'Untestable'
            elif row['DVPupilReactivityLghtLftEyeReslt'] == 'Brisk' and row[
        ...
        dummy_df.loc[self.gcs_data['PupilReactivity'].isnull(), dummy_df.columns.str.startswith('PupilReactivity' + "_")] = np.nan
        dummy_df.loc[self.gcs_data['DVGCSEyes'].isnull(), dummy_df.columns.str.startswith('GCSEye' + "_")] = np.nan
        dummy_df.loc[self.gcs_data['DVGCSMotor'].isnull(), dummy_df.columns.str.startswith('GCSMtr' + "_")] = np.nan
        dummy_df.loc[self.gcs_data['DVGCSVerbal'].isnull(), dummy_df.columns.str.startswith('GCSVrb' + "_")] = np.nan
    ...
    def vital_freq(self, hours=48, time_stamp=1):
        df_ts = pd.read_csv(DATA_PATH + "time_series_data.csv", skipinitialspace=True)
        # guid_ICU = self.clinic_data.Guid[self.clinic_data['PatientType_Hospital admit with ICU'] == 1]
        # guid_hos = self.clinic_data.Guid[self.clinic_data['PatientType_Hospital admit no ICU'] == 1]
...
def ts_builder_3d(c, file_name=DATA_PATH + "time_series_data.csv", max_len=120, masking=True):
    ...
    values = np.zeros((len(guid), num_time_steps, num_features))
    mask = np.zeros((len(guid), num_time_steps, num_features))
    ...
    for row in tqdm(df.itertuples()):
        ...
        mask[pind, tstep, vind] = 1
    
    if masking:
        return values, mask, pat_to_ind, var_to_ind
    else:
        values = np.where(mask==0, np.nan, values)
        return values, pat_to_ind, var_to_ind