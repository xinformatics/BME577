...
# # from tensorflow.python.keras.layers import _generate_dropout_mask
...
class GRUDCell(GRUCell):
    ...
    def build(self, input_shape):
        ...
        # correction for keras migration
        self.build_correction_keras_version()
    ...
    def build_correction_keras_version(self):
        '''
    ...
    def call_correction_keras_version(self):
        ...
        if self.use_bias:
            # bias for inputs
            self.input_bias_z = input_bias[:self.units]
            self.input_bias_r = input_bias[self.units: self.units * 2]
            self.input_bias_h = input_bias[self.units * 2:]
            # bias for hidden state - just for compatibility with CuDNN
            if self.reset_after:
                self.recurrent_bias_z = recurrent_bias[:self.units]
                self.recurrent_bias_r = (
                    recurrent_bias[self.units: self.units * 2])
                self.recurrent_bias_h = recurrent_bias[self.units * 2:]
        else:
    ...
    def call(self, inputs, states, training=None):
        ...
        """
        self.call_correction_keras_version()
